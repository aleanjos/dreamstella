...
public:
	...
	*/
	virtual FBInitStatus initialize(const string& title, uInt32 width, uInt32 height);
...
protected:
	...
	struct VideoMode {
		...
		// Preenche um buffer externo com as informações
		void toBuffer(char* buf, size_t size) const {
			snprintf(buf, size,
				"image_x=%u  image_y=%u  image_w=%u  image_h=%u\nscreen_w=%u  screen_h=%u",
				(unsigned int)image_x, (unsigned int)image_y, (unsigned int)image_w, (unsigned int)image_h,
