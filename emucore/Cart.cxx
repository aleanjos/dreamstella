	...
	string& dtype, string& id, const OSystem& osystem, Settings& settings)
{
	...
	if (type == "AUTO" || settings.getBool("rominfo"))
	{
		...
		if (type != "AUTO" && type != detected)
			printf("Auto-detection not consistent: %s%s", type.c_str(), detected.c_str());

		type = detected;
	}

	snprintf(buf, sizeof(buf), "%s%s", type.c_str(), autodetect.c_str());
	...
	if (type == "2IN1")
	{
		...
		if (size == 2 * 2048 || size == 2 * 4096 || size == 2 * 8192 || size == 2 * 16384)
		{
			...
			snprintf(buf + len, sizeof(buf) - len, "%s", id.c_str());
		}
		else
			dtype = "WRONG_SIZE";
	}
	...
	if (size < 1024) {
		size_t len = strlen(buf);
		snprintf(buf + len, sizeof(buf) - len, "(%uB)", size);
	}
	else {
		size_t len = strlen(buf);
		snprintf(buf + len, sizeof(buf) - len, " (%iK) ", (size / 1024));
	}
	...
	uInt32 numroms, string& md5, string& id, Settings& settings)
{
	...
	snprintf(buf, sizeof(buf), " [G%u]", (i + 1));
	id = buf;

	// Move to the next game the next time this ROM is loaded
	settings.setValue("romloadcount", (int)((i + 1) % numroms));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
const char* Cartridge::myAboutString = "";
