...
void Console::setPalette(const string &type)
{
	...
	static const uInt32 *const ALL_PALETTES[3][3] = {
		 {Console::ourNTSCPalette, Console::ourPALPalette, Console::ourSECAMPalette},
		 {Console::ourNTSCPaletteZ26, Console::ourPALPaletteZ26, Console::ourSECAMPaletteZ26}};
	...
	int formatIdx = 0;
	if (myDisplayFormat.find("PAL") != string::npos)
		formatIdx = 1;
	else if (myDisplayFormat.find("SECAM") != string::npos)
		formatIdx = 2;

	const uInt32 *selected = ALL_PALETTES[paletteNum][formatIdx];

	if (selected != nullptr)
	{
		myOSystem->frameBuffer().setTIAPalette(selected);
	}
}
...
void Console::initializeAudio()
{
	...
	myOSystem->sound().setChannels(1); // sound == "STEREO" ? 2 : 1);
...
void Console::changeYStart(int direction)
{
	...
	snprintf(val, sizeof(val), "YStart %u", ystart);
...
void Console::changeHeight(int direction)
{
	...
	snprintf(val, sizeof(val), "Height %u", height);
...
void Console::setTIAProperties()
{
		 ...
		 myDisplayFormat == "SECAM60")
	{
		...
		printf("setTIAProperties: ROM NTSC: framerate: %d\n", (int)myFramerate);
	}
	else
	{
		...
		height = BSPF_max<uInt32>(height, 250u);

		printf("setTIAProperties: ROM PAL: framerate: %d\n", (int)myFramerate);
	}
	...
	if (height > myOSystem->desktopHeight())
	{
		...
		ystart = BSPF_min<uInt32>(ystart, 64u);
...
void Console::setControllers(const string &rommd5)
{
	...
	if (BSPF_startsWithIgnoreCase(left, "PADDLES"))
	{
	...
	if (BSPF_startsWithIgnoreCase(right, "PADDLES"))
	{
		bool swapAxis = false, swapDir = false;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void Console::toggleTIABit(int bit, const string &bitname, bool show) const
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void Console::toggleTIACollision(int bit, const string &bitname, bool show) const
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
uInt32 Console::ourNTSCPalette[512] = {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
uInt32 Console::ourPALPalette[512] = {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
uInt32 Console::ourSECAMPalette[512] = {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
uInt32 Console::ourNTSCPaletteZ26[512] = {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
uInt32 Console::ourPALPaletteZ26[512] = {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
uInt32 Console::ourSECAMPaletteZ26[512] = {
