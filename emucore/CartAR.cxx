...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
CartridgeAR::CartridgeAR(const uInt8* image, uInt32 size, const Settings& settings)
   : Cartridge(settings),
   my6502(0),
   mySize(BSPF_max<uInt32>(size, 8448u))
{
   myImage = new uInt8[8192];
   myHeader = new uInt8[256];
...
void CartridgeAR::initializeROM() {
   uInt8* romPtr = myImage + (3 << 11);

   memset(romPtr, 0x02, 2048);
   memcpy(romPtr, ourDummyROMCode, sizeof(ourDummyROMCode));

   romPtr[109] = mySettings.getBool("fastscbios") ? 0xFF : 0x00;
   romPtr[281] = mySystem->randGenerator().next();
...
void CartridgeAR::loadIntoRAM(uInt8 load)
{
   ...
   printf("ERROR: Supercharger load is missing from ROM image...\n");
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
const uInt8 CartridgeAR::ourDummyROMCode[] = {
