...
Thumbulator::Thumbulator(const uInt16* rom_ptr, uInt16* ram_ptr, bool traponfatal)
	...
	ram(ram_ptr)
{
	printf("Thumbulator: Inicializando objeto Thumbulator...\n");
...
string Thumbulator::run(void)
{
	...
	{
		if (execute()) break;
		if (instructions > 500000) {
			// way more than would otherwise be possible
			printf("instructions > 500000");
			fflush(stdout);
		}
	}
#if defined(THUMB_DISS) || defined(THUMB_DBUG)
	dump_counters();
	printf(statusMsg.str());
#endif
...
{
	printf("ARM Error (%s): %X - %s\n", opcode, v1, msg);
	return 1;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
int Thumbulator::fatalError(const char* opcode, uInt32 v1, uInt32 v2, const char* msg)
{
	printf("ARM Error (%s): %X, %X - %s\n", opcode, v1, v2, msg);
	return 1;
}
...
{
	printf("\n\n instructions fetches      %u\nreads        %u\nwrites       %u\nmemcycles    %u\n%u", instructions, fetches, reads, writes, (fetches + reads + writes));
}
...
int Thumbulator::execute(void)
{
	...
	printf("Thumbulator: instrucao invalida em %08X: %04X\n", reg[15], inst);
	return(1);
}
